    ...
    for seed in xrange(N_REPEAT):
        ...
        for tr_list, te_list in kf.split(np.arange(N_sample)):
            ...
            if finetune is True:
                ...
                X_train, X_train_aug, y_train, lesion_indicator_train=extract_multiscale_patches_from_mri( \
                                                            ...
                                                            is_test=False, is_oversampling=False, row_size=row_size, \
                                                             channel_size=channel_size, num_patch=num_patch, \
                                                                proportion=proportion, n_time_point=TIME_POINT, \
                                                            fixed_width=FIXED_WIDTH, fixed_depth=FIXED_DEPTH)
                X_train, X_train_aug, y_train=preprocess(X_train, X_train_aug, y_train, mean, std, mean_aug, std_aug)
                
                # Callbacks
                SCHEDULER_FINE=lambda epoch:lr*(0.99 ** epoch)/20.0 
                info_check_string_fine='weights/{}/fine_{}_{}.hdf5'.format(log_name, seed, count_folds)
                ...
                b_generator_fine=balance_generator(X_train, X_train_aug, y_train, \
                        lesion_indicator_train, batch_size, np.mean(lesion_indicator_train))
